...
class FootballPipeline(object):
    ...
    def process_item(self, item, spider):
        self.file.writelines(
            item['team_name'] + " " + item['country'] + " " + item['founded_time'] + " " + item["league"] + " " + item["coach"] + " " + item[
                "city"] + " " + item["court"] + item["official_website"] + "\n")
        return item
...
class LocationPipeline(object):
    ...
    def process_item(self, item, spider):
        self.file.writelines(
            item['title_name1'].strip() + " " + item['title_name2'].strip() + " " + item['title_name3'].strip() + " " + item['title_name4'] + "\n")
        return item
...
class HaHaPipeline(object):
    ...
    # @check_spider_pipeline
    def process_item(self, item, spider):
        joke = item['joke_content'].strip().replace("<p>", "").replace(' ', '').replace("</p>", "").replace("&nbsp;", "").replace("<br />",
                                                                                                                                  "").replace(
            "&rdquo;", "").replace("<br/>", "").replace("&ldquo;", "").replace('&hellip;', '').replace('&mdash;', '').replace('&lsquo;', '').replace(
            '&rsquo;', '').replace('&zwj;', '').replace('<br>', '')
        clean_joke = ''
